...
public class BodyMeasureCursorAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        Date date = DateConverter.DBDateStrToDate(cursor.getString(cursor.getColumnIndex(DATABodyMeasure.DATE)));
        String dateStr = DateConverter.dateToLocalDateStr(date, mContext);
        t1.setText(dateStr);

        float measure = cursor.getFloat(cursor.getColumnIndex(DATABodyMeasure.MEASURE));
        Unit unit = Unit.fromInteger(cursor.getInt(cursor.getColumnIndex(DATABodyMeasure.UNIT)));
        ...
        editImg.setTag(cursor.getLong(cursor.getColumnIndex(DATABodyMeasure.KEY)));
        ...
        deletImg.setTag(cursor.getLong(cursor.getColumnIndex(DATABodyMeasure.KEY)));
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        return mInflater.inflate(R.layout.body_measure, parent, false);
    }
