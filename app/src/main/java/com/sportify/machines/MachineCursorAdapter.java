...
public class MachineCursorAdapter extends CursorAdapter implements Filterable {
    ...
    public MachineCursorAdapter(Context context, Cursor c, int flags, DATAMachine pDbMachine) {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        t0.setText(cursor.getString(cursor.getColumnIndex(DATAMachine.KEY)));

        TextView t1 = view.findViewById(R.id.LIST_MACHINE_NAME);
        t1.setText(cursor.getString(cursor.getColumnIndex(DATAMachine.NAME)));

        TextView t2 = view.findViewById(R.id.LIST_MACHINE_SHORT_DESCRIPTION);
        t2.setText(cursor.getString(cursor.getColumnIndex(DATAMachine.DESCRIPTION)));

        ImageView i0 = view.findViewById(R.id.LIST_MACHINE_PHOTO);
        String lPath = cursor.getString(cursor.getColumnIndex(DATAMachine.PICTURE));

        ExerciseType lType = ExerciseType.fromInteger(cursor.getInt(cursor.getColumnIndex(DATAMachine.TYPE)));
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        return mInflater.inflate(R.layout.exercise_list, parent, false);
